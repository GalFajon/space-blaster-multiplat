...
public class EnemySoldierD : Enemy, IPathFollower, IAnimatable
{
    ...
    public EnemySoldierD(WeaponStats weaponstats, float x, float y, SpaceBlaster.Scene.Scene scene, SceneObject parent = null) : base(4, x, y, new Rectangle(0, 0, 32, 32), true, scene, parent)
    {
    ...
    public override void HandleAI(GameTime gameTime, Scene level)
    {
        base.HandleAI(gameTime, level);

        if (level is Level l && !this.IsHit)
        {
